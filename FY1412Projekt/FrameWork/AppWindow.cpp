...
AppWindow::AppWindow(Settings settings)
{
...
void AppWindow::setWindowPosition(vec pos)
{
	this->appWindow.pos = pos;
}
void AppWindow::setWindowSize(vec size)
{
	this->appWindow.size = size;
}
void AppWindow::setState(int state)
{
	this->appWindow.state = state;
}

void AppWindow::setName(LPCTSTR name)
{
	this->appWindow.name = name;
}


bool AppWindow::createClass(HINSTANCE hInstance, WNDPROC wndProc)
{
...
bool AppWindow::createWindow(HWND& hwnd, HINSTANCE hInstance, int nCmdShow, Settings* settings)
{
	DWORD style;
	vec pos = { 0,0 };
	vec addSize = { 0,0 };
...
bool AppWindow::updateWindow(HWND& hwnd, HINSTANCE hInstance, int nCmdShow, Settings* settings)
{
	DestroyWindow(hwnd);
	hwnd = NULL;
